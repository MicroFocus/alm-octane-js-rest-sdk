...
class Octane {
  ...
  prepareApi (struct, baseType) {
    ...
    Object.keys(struct).forEach(function (routePart) {
      ...
      if (block.url && block.params) {
        ...
        let section = utils.toCamelCase(parts[1].toLowerCase(), true)
        parts.splice(0, 2)
        let funcName = utils.toCamelCase(parts.join('-'), true)
        ...
        self[section][funcName] = function (msg, callback) {
          ...
          if (!self.requestor) {
            let err = new error.Unauthorized('authentication is required')
  ...
  sanitizeParam (paramName, value, definition) {
    ...
    switch (definition.type) {
      ...
      case 'reference':
        let ref
        let multiple = definition.field_type_data.multiple
        if (multiple) {
    ...
    this.httpSend(msg, block, (err, response, body) => {
      ...
      if (response.headers['content-type'] === 'application/octet-stream') {
        ...
        if ('total_count' in ret && 'data' in ret) {
          let totalCount = ret.total_count
          ret = ret.data
  ...
  convertBlockToBody (msg, block) {
    ...
    Object.keys(block.params).forEach(function (paramName) {
      ...
      let paramValue = msg[paramName]
      let isUrlParam = block.url.indexOf(':' + paramName) !== -1
      if (isUrlParam) {
  ...
  httpSend (msg, block, callback) {
    const self = this
    let body = {}
    let method = block.method.toUpperCase()
    let accept = block.accept
    let contentType = block['content-type']
    ...
    if (Object.keys(body)) {
      if ('HEAD|GET|DELETE'.indexOf(method) !== -1) {
        ...
        let content = body.file
        delete body.file
        let entity = JSON.stringify(body)
        options.formData = {
          entity: {
            value: entity,
            options: {
