...
Octane.prototype.authenticate = function authenticate (options, callback) {
  ...
  var opt = {
    ...
    headers: { HPECLIENTTYPE: 'HPE_REST_API_TECH_PREVIEW' }
  }
...
Octane.prototype.parseParams = function parseParams (msg, paramsStruct) {
  ...
  for (var i = 0, l = params.length; i < l; i++) {
    ...
    value = this.verifyParam(paramName, value, def)

    msg[paramName] = value
  }
}

Octane.prototype.verifyParam = function verifyParam (paramName, value, definition) {
  if (!definition.type) {
    return value
  }
...
Octane.prototype.handler = function handler (msg, block, callback) {
  ...
  this.httpSend(msg, block, function (err, response, body) {
    ...
    if (response.headers['content-type'] === 'application/octet-stream') {
      ret = body
    } else if (response.req.method === 'GET') {
