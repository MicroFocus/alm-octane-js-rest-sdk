...
class Octane {
  ...
  authenticate (options, callback) {
    ...
    requestor.post({
      ...
      if (response.statusCode >= 400) {
        err = error(statusCode, body, response.headers)
        debug('authentication failed %j', err)
        return callback(err)
      }

      baseUrl = `${baseUrl}/api/shared_spaces/${self.config.shared_space_id}/workspaces/${self.config.workspace_id}`

      requestor = requestor.defaults({
        baseUrl: baseUrl
      })

      if (self.config.alm_octane_tech_preview) {
        requestor = requestor.defaults({ headers: { ALM_OCTANE_TECH_PREVIEW : true } })
      } 
      if(self.config.hpeclienttype) {
        requestor = requestor.defaults({ headers: { HPECLIENTTYPE: self.config.hpeclienttype } })
      }
  ...
  prepareApi (struct, baseType = '') {
    ...
    Object.keys(struct).forEach(routePart => {
      ...
      if (block.url && block.params) {
        ...
        if (!self[section]) {
          self[section] = {}
          self[utils.toCamelCase(`get-${section}-api`, true)] = () => self[section]
        }

        self[section][funcName] = (msg, callback) => {
  ...
  sanitizeParam (paramName, value, definition) {
    ...
    switch (definition.type) {
      ...
      default:
        throw new error.BadRequest(
          `Unknown parameter type for parameter '${paramName}': ${definition.type}`
        )
    }

    if (isInvalid) {
      throw new error.BadRequest(
        `Invalid value for parameter '${paramName}': ${typeof value}` === 'object' ? JSON.stringify(value) : value
      )
    }
  ...
  convertBlockToBody (msg, block) {
    ...
    Object.keys(block.params).forEach(paramName => {
      ...
      const isUrlParam = block.url.includes(`:${paramName}`)
      if (isUrlParam) {
