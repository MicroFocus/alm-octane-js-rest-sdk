...
class Octane {
  ...
  /**
   *
   * @param customUrl Url to send the request to
  ...
  async executeCustomRequest (customUrl, operation, body = undefined, headers = undefined) {
    ...
    if (body) {
      response = this._requestHandler[operation](customUrl, body, config)
    } else {
      response = this._requestHandler[operation](customUrl, config)
    }
