...
describe('query', () => {
  ...
  it('should test "no vlaue"', () => {
    ...
    const query = Query.field('user_tags').equal(Query.field('id').equal(1001)).or().field('user_tags').equal(Query.NONE)
    assert.strictEqual(query.build(), expect)
  })
  ...
  it('should test complex statement or', () => {
    ...
    const query = Query.field('created_name').less(now).or(Query.field('id').equal(5028)).or(Query.field('id').equal(5015))
    assert.strictEqual(query.build(), expect)

    const query2 = Query.field('created_name').less(now).or().field('id').equal(5028).or().field('id').equal(5015)
    assert.strictEqual(query2.build(), expect)
  })

  it('should test complex statement and negate', () => {
    const expect = '!id GE 5028;!name EQ ^test^'
    const query = Query.field('id').notGreaterEqual(5028).and(Query.field('name').notEqual('test'))
    assert.strictEqual(query.build(), expect)

    const query2 = Query.field('id').notGreaterEqual(5028).and().field('name').notEqual('test')
    assert.strictEqual(query2.build(), expect)
  })

  it('should test complex statement and negate and group', () => {
    const expect = '!(id GE 5028);!(name EQ ^test^)'
    const query = Query.field('id').greaterEqual(5028).group().not().and(Query.field('name').equal('test').group().not())
    assert.strictEqual(query.build(), expect)
  })

  it('should test complex statement with reference', () => {
    const expect = 'user_tags EQ {id EQ 1001};user_tags EQ {id EQ 50000000}'
    const query = Query.field('user_tags').equal(Query.field('id').equal(1001)).and(Query.field('user_tags').equal(Query.field('id').equal(50000000)))
    assert.strictEqual(query.build(), expect)

    const query2 = Query.field('user_tags').equal(Query.field('id').equal(1001)).and().field('user_tags').equal(Query.field('id').equal(50000000))
    assert.strictEqual(query2.build(), expect)
  })
